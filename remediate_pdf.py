...
def filter_page_content(page_obj, complex_bboxes_pdf_space):
    """
    Parses the page content stream. Strips out all path drawing operators (vectors),
    and filters out text showing operators (Tj/TJ) inside complex component bounding boxes.
    Yields blocks of instructions.
    """
    try:
    ...
    for operands, operator in instructions:
        ...
        # Strip path operators
        if op_name in path_operators:
            continue
        ...
        if in_text_block:
            ...
            if op_name == 'Tm':
                ...
                # Check if current (tx, ty) is inside any complex bounding box
                inside = False
                ...
                if inside:
                    ...
                    # Validate if there is visible text
                    if op_name == 'Tj':
