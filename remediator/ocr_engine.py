...
def generate_ocr_text_ops(
    ...
    try:
        ...
        if "/Helvetica" not in pikepage.Resources.Font:
            helvetica_font = pdf_doc.make_indirect(
                pikepdf.Dictionary(
        ...
        for i in range(n_boxes):
            ...
            conf = (
                int(ocr_data["conf"][i])
                if "conf" in ocr_data and str(ocr_data["conf"][i]).lstrip("-").isdigit()
                else 0
            )
            ...
            if word_str and conf > 20:
                ...
                words.append({"text": word_str, "x": pdf_x, "y": pdf_y, "font_size": max(8.0, h)})
        ...
        for word in words:
            ...
            ocr_ops.append(
                ([pikepdf.Name("/P"), pikepdf.Dictionary(MCID=mcid)], pikepdf.Operator("BDC"))
            )
